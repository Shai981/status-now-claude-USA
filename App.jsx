...
/* ============================================================
   Status Now — US Edition
   English-first, LTR, location-based real-time social network.
   Built for the US market. Single-file MVP.
   ============================================================ */
...
const CATEGORIES = {
  traffic:  { he: "Traffic & Jams",   short: "Traffic",  color: "#F59E0B", Icon: Car },
  security: { he: "Safety Alert",     short: "Safety",   color: "#EF4444", Icon: Shield },
  weather:  { he: "Weather",          short: "Weather",  color: "#3BA3E8", Icon: CloudRain },
  event:    { he: "Event",            short: "Event",    color: "#8B5CF6", Icon: CalendarDays },
  crowd:    { he: "Crowds & Waits",   short: "Crowds",   color: "#F97316", Icon: Users2 },
  road:     { he: "Road Issue",       short: "Road",     color: "#A16207", Icon: Construction },
  general:  { he: "General Update",   short: "General",  color: "#64748B", Icon: Info },
};
...
function timeAgo(ts) {
  const d = now() - ts;
  if (d < 0) return "in a moment";
  if (d < MIN) return "just now";
  if (d < HR) { const m = Math.floor(d / MIN); return `ago ${m} min`; }
  if (d < 24 * HR) { const h = Math.floor(d / HR); return `ago ${h} hr`; }
  const dd = Math.floor(d / (24 * HR)); return `ago ${dd} days`;
}
...
const MAP_W = 500, MAP_H = 320;
const LNG_MIN = -125.0, LNG_MAX = -66.5, LAT_MIN = 24.5, LAT_MAX = 49.5;
...
const PHOTOS = {
  // Ayalon Highway, Tel Aviv — CC BY-SA, Wikimedia Commons
  ...
  // Mahane Yehuda Market, Jerusalem — CC BY-SA 3.0, Deror avi
  ...
  // Israeli road — PikiWiki CC BY 2.5
...
function seedData() {
  const users = [
    { id: "u_me",  name: "Alex Rivera",  color: AVCOLORS[0], bio: "City lover, coffee addict & real-time info junkie ☕", city: "New York",     lat: CITIES[0].lat, lng: CITIES[0].lng, reputation: 128 },
    { id: "u_dan", name: "Dan Cooper",   color: AVCOLORS[1], bio: "Truck driver, reporting from the highways", city: "Los Angeles",  lat: CITIES[1].lat, lng: CITIES[1].lng, reputation: 214 },
    { id: "u_maya",name: "Maya Chen",    color: AVCOLORS[2], bio: "Student in Chicago, always on the move", city: "Chicago",      lat: CITIES[2].lat, lng: CITIES[2].lng, reputation: 63 },
    { id: "u_avi", name: "Avi Patel",    color: AVCOLORS[3], bio: "Houston local, weather watcher", city: "Houston",      lat: CITIES[3].lat, lng: CITIES[3].lng, reputation: 91 },
    { id: "u_tal", name: "Tal Morgan",   color: AVCOLORS[4], bio: "Morning runner in Miami Beach parks", city: "Miami",        lat: CITIES[9].lat, lng: CITIES[9].lng, reputation: 45 },
  ];

  const t = now();
  const statuses = [
    { id: uid(), userId: "u_dan", category: "traffic", text: "Heavy traffic on the 405 northbound near Santa Monica — completely gridlocked. Use surface streets.", locationName: "I-405 near Santa Monica, LA", lat: 34.01, lng: -118.49, createdAt: t - 8 * MIN, expiresAt: t + 52 * MIN, likes: ["u_maya", "u_tal", "u_avi"], comments: [{ id: uid(), userId: "u_tal", text: "Thanks! Taking Sepulveda instead", createdAt: t - 5 * MIN }], media: { type: "image", src: PHOTOS.traffic, caption: "Ayalon Highway, Tel Aviv" } },
    { id: uid(), userId: "u_maya", category: "crowd", text: "Huge line at Whole Foods on Columbus Circle, about 20 min wait at checkout.", locationName: "Whole Foods Columbus Circle, NYC", lat: 40.768, lng: -73.983, createdAt: t - 22 * MIN, expiresAt: t + 38 * MIN, likes: ["u_me"], comments: [], media: { type: "image", src: PHOTOS.market, caption: "Mahane Yehuda Market" } },
    { id: uid(), userId: "u_avi", category: "weather", text: "Heavy rain started in downtown Chicago, bring an umbrella. Low visibility on Michigan Ave.", locationName: "Michigan Ave, Chicago", lat: 41.878, lng: -87.624, createdAt: t - 40 * MIN, expiresAt: t + 80 * MIN, likes: ["u_me", "u_dan"], comments: [], media: { type: "image", src: PHOTOS.rain, caption: "Haifa, Israel" } },
    { id: uid(), userId: "u_tal", category: "event", text: "Concert at Central Park SummerStage starts at 8PM — incredible vibe out here 🎶", locationName: "Central Park SummerStage, NYC", lat: 40.779, lng: -73.970, createdAt: t - 70 * MIN, expiresAt: t + 3 * HR, likes: ["u_me", "u_maya", "u_dan", "u_avi"], comments: [{ id: uid(), userId: "u_me", text: "On my way!", createdAt: t - 60 * MIN }], media: { type: "image", src: PHOTOS.concert, caption: "Herzliya Marina" } },
    { id: uid(), userId: "u_dan", category: "road", text: "Big pothole on I-10 near Downtown Houston exit, watch the right lane.", locationName: "I-10 near Downtown Houston", lat: 29.752, lng: -95.367, createdAt: t - 95 * MIN, expiresAt: t + 5 * HR, likes: [], comments: [], media: { type: "image", src: PHOTOS.road, caption: "Israeli Highway" } },
    { id: uid(), userId: "u_me", category: "general", text: "Grand opening of new coffee shop on 5th Ave — first week BOGO on all lattes! ☕", locationName: "5th Ave, Manhattan, NYC", lat: 40.754, lng: -73.975, createdAt: t - 3 * HR, expiresAt: t + 6 * HR, likes: ["u_maya"], comments: [], media: { type: "image", src: PHOTOS.cafe, caption: "Rothschild Blvd, Tel Aviv" } },
    { id: uid(), userId: "u_avi", category: "security", text: "Heavy police presence near Grand Central Station, best to use alternate routes.", locationName: "Grand Central Station, NYC", lat: 40.752, lng: -73.977, createdAt: t - 4 * HR, expiresAt: t - 10 * MIN /* expired */, likes: ["u_me"], comments: [], media: { type: "image", src: PHOTOS.security, caption: "Haifa Bay" } },
  ];

  const requests = [
    { id: uid(), userId: "u_me", category: "crowd", text: "Anyone know how crowded Coney Island Beach is right now? Worth the trip?", locationName: "Coney Island Beach, NYC", lat: 40.575, lng: -73.985, createdAt: t - 15 * MIN, status: "open", answers: [] },
    { id: uid(), userId: "u_tal", category: "traffic", text: "Any traffic on I-95 north out of Miami right now?", locationName: "I-95 North, Miami", lat: 25.820, lng: -80.132, createdAt: t - 35 * MIN, status: "answered", answers: [{ id: uid(), userId: "u_dan", text: "Flowing fine right now, clear all the way to Fort Lauderdale.", createdAt: t - 20 * MIN }] },
    { id: uid(), userId: "u_maya", category: "general", text: "Any pharmacy open right now in the West Village area?", locationName: "West Village, NYC", lat: 40.734, lng: -74.005, createdAt: t - 2 * HR, status: "open", answers: [] },
    { id: uid(), userId: "u_avi", category: "weather", text: "Is it raining in Midtown right now? About to head out", locationName: "Midtown Manhattan, NYC", lat: 40.754, lng: -73.984, createdAt: t - 5 * HR, status: "expired", answers: [] },
  ];

  const groups = [
    { id: "g_tlv", name: "NYC Right Now", description: "Everything happening in the city in real time — traffic, events, openings.", cover: { emoji: "🌆", color: "#1F5EFF" }, privacy: "public", memberIds: ["u_me", "u_dan", "u_tal"] },
    { id: "g_jlm", name: "Chicago Live", description: "Real-time updates from the Loop to the neighborhoods.", cover: { emoji: "🏙️", color: "#E8A32B" }, privacy: "public", memberIds: ["u_maya"] },
    { id: "g_roads", name: "US Highways 🚗", description: "Traffic reports, road issues and closures from coast to coast.", cover: { emoji: "🚗", color: "#12A594" }, privacy: "public", memberIds: ["u_dan", "u_avi", "u_me"] },
    { id: "g_haifa", name: "Houston Heights", description: "Private neighborhood community for Heights residents.", cover: { emoji: "🏘️", color: "#8B5CF6" }, privacy: "private", memberIds: ["u_avi"] },
    { id: "g_run", name: "Morning Runners", description: "Weather, routes and park conditions for early birds.", cover: { emoji: "🏃", color: "#F97316" }, privacy: "public", memberIds: ["u_tal"] },
  ];
    ...
    <div style={{ minHeight: "100%", display: "flex", flexDirection: "column", justifyContent: "center", padding: "28px 22px" }}>
      <div style={{ textAlign: "center", marginBottom: 26 }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 9, fontWeight: 900, fontSize: 30, letterSpacing: "-0.5px", color: T.ink }}>
          ...
          <span>Status<span style={{ color: T.brand }}>&nbsp;Now</span></span>
        </div>
        <div style={{ color: T.inkSoft, marginTop: 8, fontSize: 14 }}>What's happening right now — right near you.</div>
      </div>
...
function Login({ go, onLogin, toast }) {
  ...
  return (
    <AuthShell>
      <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 2, color: T.ink }}>Sign in</div>
      <div style={{ fontSize: 13, color: T.inkSoft, marginBottom: 20 }}>Welcome back 👋</div>

      <SocialBtn color="#DB4437" onClick={() => { setLoading(true); setTimeout(() => { setLoading(false); onLogin({ name:"Google User", provider:"google" }); }, 600); }}
      ...
      >Anonymous sign in</SocialBtn>

      <div style={{ display:"flex", alignItems:"center", gap:10, margin:"4px 0 14px" }}>
        <div style={{ flex:1, height:1, background:T.line }}/><span style={{ fontSize:12.5, color:T.inkFaint }}>or or Email</span><div style={{ flex:1, height:1, background:T.line }}/>
      </div>
      ...
      <Field label="Password">
        <div style={{ position: "relative" }}>
          <input style={{ ...inputStyle, paddingInlineEnd: 42 }} type={show ? "text" : "password"} value={pw} onChange={(e) => setPw(e.target.value)} />
      ...
      <div style={{ textAlign: "end", marginBottom: 14 }}>
        <button onClick={() => go("forgot")} style={{ border: "none", background: "none", color: T.brand, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Forgot password?</button>
      </div>
      <PrimaryButton onClick={submit} disabled={loading}>{loading ? "Signing in…" : "Sign in with Email"}</PrimaryButton>
      <div style={{ textAlign: "center", marginTop: 16, fontSize: 13.5, color: T.inkSoft }}>
        Don't have an account? <button onClick={() => go("register")} style={{ border: "none", background: "none", color: T.brand, fontWeight: 800, cursor: "pointer" }}>Sign up</button>
      </div>
    </AuthShell>
  );
}
...
const ANON_NAMES = ["Swift Fox","Blue Heron","Quiet Panda","Silver Wolf","Amber Hawk","Dark Horse","Night Owl","Free Eagle","Cool Breeze","Lone Star","Red Maple","Wild River","Storm Cloud","Ocean Wave","Desert Sage"];
...
function Register({ go, onLogin }) {
  ...
  const submitEmail = () => {
    setErr("");
    if (f.name.trim().length < 2) return setErr("Please enter a name or nickname (min 2 chars)");
    if (!f.email.includes("@")) return setErr("Invalid email address");
    if (f.pw.length < 6) return setErr("Password too short (min 6 chars)");
  ...
  return (
    <AuthShell>
      <div style={{ fontSize: 19, fontWeight: 900, marginBottom: 2 }}>Sign up</div>
      <div style={{ fontSize: 13, color: T.inkSoft, marginBottom: 20 }}>Join the community that updates in real time</div>

      {mode === "choose" ? (
        <>
          {/* Google */}
          <SocialBtn color="#DB4437" onClick={() => loginWith("Google User", "google")}
            icon={<svg width="20" height="20" viewBox="0 0 48 48"><path fill="#EA4335" d="M24 9.5c3.5 0 6.6 1.2 9 3.2l6.7-6.7C35.8 2.5 30.3 0 24 0 14.6 0 6.6 5.5 2.7 13.5l7.8 6C12.4 13.1 17.7 9.5 24 9.5z"/><path fill="#4285F4" d="M46.5 24.5c0-1.6-.1-3.1-.4-4.5H24v8.5h12.7c-.6 3-2.3 5.5-4.8 7.2l7.5 5.8c4.4-4.1 7.1-10.1 7.1-17z"/><path fill="#FBBC05" d="M10.5 28.6A14.5 14.5 0 0 1 9.5 24c0-1.6.3-3.2.8-4.6l-7.8-6A24 24 0 0 0 0 24c0 3.9.9 7.5 2.7 10.7l7.8-6.1z"/><path fill="#34A853" d="M24 48c6.2 0 11.5-2 15.3-5.5l-7.5-5.8c-2.1 1.4-4.7 2.3-7.8 2.3-6.3 0-11.6-3.6-13.5-8.8l-7.8 6.1C6.6 42.5 14.6 48 24 48z"/></svg>}
          >Continue with Google</SocialBtn>
          ...
          <SocialBtn color="#0D9488" onClick={() => loginWith(randomAnon(), "anon")}
            icon={<span style={{fontSize:18}}>🎭</span>}
          >Anonymous entry (random nickname)</SocialBtn>

          {/* Divider */}
          <div style={{ display:"flex", alignItems:"center", gap:10, margin:"4px 0 14px" }}>
            <div style={{ flex:1, height:1, background:T.line }}/>
            <span style={{ fontSize:12.5, color:T.inkFaint }}>or</span>
            <div style={{ flex:1, height:1, background:T.line }}/>
          </div>
          ...
          <div style={{ textAlign: "center", marginTop: 16, fontSize: 13.5, color: T.inkSoft }}>
            Already have an account? <button onClick={() => go("login")} style={{ border:"none", background:"none", color:T.brand, fontWeight:800, cursor:"pointer" }}>Sign in</button>
          </div>
        </>
      ) : (
        <>
          ...
          <Field label="Name or nickname" hint="Shown on your posts — nickname is fine">
            <input style={inputStyle} value={f.name} onChange={up("name")} placeholder="e.g. Alex K., Morning Runner, anon42" />
          </Field>
          <Field label="Email"><input style={inputStyle} value={f.email} onChange={up("email")} placeholder="name@example.com" /></Field>
          <Field label="Password"><input style={inputStyle} type="password" value={f.pw} onChange={up("pw")} /></Field>
          <Field label="Verify Password"><input style={inputStyle} type="password" value={f.pw2} onChange={up("pw2")} /></Field>
          ...
          <PrimaryButton onClick={submitEmail} disabled={loading}>{loading ? "Creating account…" : "Create account"}</PrimaryButton>
        </>
      )}
    </AuthShell>
  );
...
function Forgot({ go }) {
  ...
  return (
    <AuthShell>
      ...
      {!sent ? (
        <>
          <div style={{ fontSize: 13, color: T.inkSoft, marginBottom: 18 }}>We'll send you a password reset link</div>
          <Field label="Email"><input style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" /></Field>
          <PrimaryButton onClick={submit} disabled={loading || !email.includes("@")}>{loading ? "Sending…" : "Send Reset Link"}</PrimaryButton>
        </>
      ) : (
        <>
          ...
          <PrimaryButton onClick={() => go("reset")}>I have a code — Reset now</PrimaryButton>
        </>
      )}
    </AuthShell>
  );
...
function Reset({ go, toast }) {
  ...
  const submit = () => {
    ...
    setLoading(true);
    setTimeout(() => { setLoading(false); toast("Password reset successfully"); go("login"); }, 700);
  };
  return (
    <AuthShell>
    ...
    <div className="sn-card" style={{ opacity: expired ? 0.65 : 1, position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12.5, color: T.inkSoft, marginTop: 2, flexWrap: "wrap" }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 3 }}><Clock size={12} /> {expired ? "Expired" : timeAgo(s.createdAt)}</span>
              <span>·</span>
        ...
        <div style={{ position: "relative", background: "#000" }}>
          ...
          {s.media.type === "video" && (
            <span style={{ position: "absolute", top: 8, insetInlineStart: 8, background: "rgba(0,0,0,.65)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 9px", borderRadius: 999, display: "inline-flex", alignItems: "center", gap: 4 }}>
              <Video size={11} /> {s.media.duration ? `${s.media.duration}s` : "video"}
            </span>
          )}
        </div>
      )}
      ...
      {(s.likes.length > 0 || s.comments.length > 0) && (
        <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 16px", fontSize: 13.5, color: T.inkSoft }}>
          ...
          {s.comments.length > 0 && <span onClick={onOpen} style={{ cursor: "pointer" }}>{s.comments.length} comments</span>}
        </div>
      )}
      ...
      <div style={{ display: "flex", borderTop: `1px solid ${T.line}`, margin: "0 16px", padding: "4px 0" }}>
        <button className={`sn-react-btn${liked ? " active" : ""}`} onClick={() => onLike(s.id)}>
          <CheckCircle2 size={18} strokeWidth={2.2} /> Verify
        </button>
        <button className="sn-react-btn" onClick={onOpen}>
          <MessageCircle size={18} strokeWidth={2.2} /> Comment
        </button>
        <button className="sn-react-btn" onClick={onOpen}>
          <Send size={18} strokeWidth={2.2} /> Share
        </button>
      </div>
    </div>
  );
...
function RequestCard({ r, users, me, dist, onOpen, onReport, onUser }) {
  ...
  const statusMap = { open: { t: "Open", color: T.brand }, answered: { t: "Answered", color: "#12A594" }, expired: { t: "Expired", color: T.inkFaint } };
    ...
    <div className="sn-card" style={{ opacity: r.status === "expired" ? 0.65 : 1, borderInlineStart: `4px solid ${c.color}` }}>
      ...
      <div style={{ display: "flex", borderTop: `1px solid ${T.line}`, margin: "0 16px", padding: "4px 0" }}>
        <button className="sn-react-btn active" style={{ color: c.color }} onClick={onOpen}>
          <Send size={18} strokeWidth={2.2} /> {r.answers.length ? `${r.answers.length} Answers` : "Answer now"}
        </button>
        <button className="sn-react-btn" onClick={onOpen}>
          <MessageCircle size={18} strokeWidth={2.2} /> Comment
        </button>
    ...
    <div>
      ...
      {newCount > 0 && (
        <button onClick={onRefresh} style={{
          ...
          <Sparkles size={16} /> {newCount} new update{newCount !== 1 ? "s" : ""} — tap to refresh
        </button>
      )}
  ...
  return (
    <div>
      <div className="sn-scroll-x" style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 10 }}>
        <button onClick={() => setShowStat(!showStat)} style={catBtn(showStat, T.brand)}><Radio size={13} /> Updates</button>
        <button onClick={() => setShowReq(!showReq)} style={catBtn(showReq, "#12A594")}><HelpCircle size={13} /> Requests</button>
      ...
      <div style={{ position: "relative", background: "linear-gradient(180deg,#EAF1FF,#F0F5EC)", borderRadius: 20, border: `1px solid ${T.line}`, overflow: "hidden", boxShadow: "inset 0 2px 12px rgba(20,22,28,.05)" }}>
        <svg viewBox={`0 0 ${MAP_W} ${MAP_H}`} width="100%" style={{ display: "block", maxHeight: "58vh" }}>
          ...
          {/* land */}
          <path d={ISRAEL_PATH} fill="#DBEAFE" stroke="#93C5FD" strokeWidth="2" />
          {/* subtle city labels */}
          {CITIES.slice(0, 6).map((c) => { const p = project(c.lat, c.lng); return (
            <text key={c.name} x={p.x} y={p.y - 9} fontSize="8.5" fill="#64748B" textAnchor="middle" style={{ pointerEvents: "none" }}>{c.name}</text>
          ); })}
          {/* me */}
        ...
        <div style={{ position: "absolute", top: 12, insetInlineStart: 12, background: "rgba(255,255,255,.9)", borderRadius: 12, padding: "8px 11px", fontSize: 11.5, color: T.inkSoft, fontWeight: 600, backdropFilter: "blur(4px)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6 }}><span style={{ width: 10, height: 10, borderRadius: 10, background: T.brand, display: "inline-block", border: "2px solid #fff" }} /> Active update</div>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}><span style={{ width: 9, height: 9, background: "#fff", border: `2.5px solid #12A594`, display: "inline-block", transform: "rotate(45deg)" }} /> Ask the Community</div>
        </div>
      </div>
      ...
      {!selItem && <div style={{ textAlign: "center", color: T.inkFaint, fontSize: 13, marginTop: 14 }}>Tap a marker on the map to see details</div>}
    </div>
  );
}
...
function Requests({ data, me, myLoc, openRequest, report, openUser }) {
  ...
  const tabs = [{ k: "open", t: "Open" }, { k: "answered", t: "Answered" }, { k: "all", t: "All" }];
  return (
    <div>
  ...
  return (
    <div>
      <div style={{ position: "relative", marginBottom: 14 }}>
        ...
        <input style={{ ...inputStyle, paddingInlineStart: 40 }} placeholder="Search groups…" value={q} onChange={(e) => setQ(e.target.value)} />
      </div>
      <PrimaryButton onClick={openCreateGroup} style={{ marginBottom: 20 }}><Plus size={18} /> Create New Group</PrimaryButton>

      {mine.length > 0 && <SectionTitle>My Groups</SectionTitle>}
      {mine.map((g) => <GroupCard key={g.id} g={g} me={me} onJoin={onJoin} openGroup={openGroup} />)}

      <SectionTitle>Discover Groups</SectionTitle>
      {others.length === 0 ? <Empty icon="🔍" title="No groups found" /> : others.map((g) => <GroupCard key={g.id} g={g} me={me} onJoin={onJoin} openGroup={openGroup} />)}
    </div>
  );
}
  ...
  return (
    <div style={{ background: T.surface, borderRadius: 18, border: `1px solid ${T.line}`, padding: 13, marginBottom: 11, display: "flex", gap: 13, alignItems: "center" }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }} onClick={() => openGroup(g.id)}>
        ...
        <div style={{ fontSize: 11.5, color: T.inkFaint, marginTop: 3 }}>{g.memberIds.length} members</div>
      </div>
  ...
  return (
    <div>
      ...
      <div style={{ background: T.surface, borderRadius: 20, border: `1px solid ${T.line}`, padding: 16, marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 12, color: T.inkFaint, marginTop: 8 }}>{g.memberIds.length} members · {posts.length} Updates</div>
      </div>
      ...
      {joined && (
        <button onClick={() => onPost(g.id)} style={{ ...chipBtn(false), width: "100%", justifyContent: "center", padding: "13px", marginBottom: 16, borderStyle: "dashed", color: T.brand, borderColor: T.brand }}>
          <Plus size={17} style={{ verticalAlign: -3 }} /> Post to this group
        </button>
      )}

      {posts.length === 0
        ? <Empty icon="💬" title="No posts in this group yet" sub={joined ? "Be the first to post!" : "Join to post and follow."} />
...
function Profile({ user, data, me, myLoc, onLogout, openStatus, openRequest, openGroup, isMe }) {
  ...
  const repTier = rep > 150 ? "Top Reporter" : rep > 80 ? "Active Contributor" : "Participant";
    ...
    <div>
      <div style={{ background: T.surface, borderRadius: 22, border: `1px solid ${T.line}`, padding: 20, marginBottom: 16, textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 16, justifyContent: "center" }}>
          <Stat n={myStatuses.length} l="Updates" />
          <Stat n={myReqs.length} l="Requests" />
          <Stat n={myGroups.length} l="Groups" />
        </div>
        ...
        <div style={{ marginTop: 16, background: `${T.gold}14`, borderRadius: 14, padding: 13, display: "flex", alignItems: "center", gap: 11, textAlign: "start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: T.inkSoft }}>Trust score {rep} · Based on verifications & helpful answers</div>
          </div>
          <TrendingUp size={18} color={T.gold} />
        </div>

        {isMe && <button onClick={onLogout} style={{ marginTop: 16, border: `1.5px solid ${T.line}`, background: T.surface, color: T.live, borderRadius: 12, padding: "10px 16px", fontWeight: 800, fontSize: 13.5, cursor: "pointer", display: "inline-flex", alignItems: "center", gap: 7, fontFamily: "inherit" }}><LogOut size={16} /> Sign out</button>}
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        {[{ k: "statuses", t: "Updates" }, { k: "requests", t: "Requests" }, { k: "groups", t: "Groups" }].map((x) =>
          <button key={x.k} onClick={() => setTab(x.k)} style={{ ...chipBtn(tab === x.k), flex: 1, justifyContent: "center" }}>{x.t}</button>)}
      </div>

      {tab === "statuses" && (myStatuses.length ? myStatuses.map((s) => <StatusCard key={s.id} s={s} users={data.users} me={me} dist={myLoc ? haversineKm(myLoc, s) : null} onLike={() => {}} onOpen={() => openStatus(s.id)} onReport={() => {}} onUser={() => {}} />) : <Empty icon="📝" title="אין Updates עדיין" />)}
      {tab === "requests" && (myReqs.length ? myReqs.map((r) => <RequestCard key={r.id} r={r} users={data.users} me={me} dist={myLoc ? haversineKm(myLoc, r) : null} onOpen={() => openRequest(r.id)} onReport={() => {}} onUser={() => {}} />) : <Empty icon="🙋" title="אין Requests עדיין" />)}
      {tab === "groups" && (myGroups.length ? myGroups.map((g) => <GroupCard key={g.id} g={g} me={me} onJoin={() => {}} openGroup={openGroup} />) : <Empty icon="👥" title="עדיין לא בGroups" />)}
    </div>
  );
}
...
function StatusDetail({ s, data, me, onClose, onLike, onComment, onUser }) {
  ...
  return (
    <Sheet onClose={onClose} title="Update">
      <StatusCard s={s} users={data.users} me={me} dist={null} onLike={onLike} onOpen={() => {}} onReport={() => {}} onUser={onUser} />
      <SectionTitle>comments</SectionTitle>
      {s.comments.length === 0 && <div style={{ color: T.inkFaint, fontSize: 13.5, padding: "4px 2px 12px" }}>No comments yet. Be the first.</div>}
      ...
      <Composer value={txt} setValue={setTxt} placeholder="Add a comment…" onSend={() => { if (txt.trim()) { onComment(s.id, txt.trim()); setTxt(""); } }} />
    </Sheet>
  );
}
...
function RequestDetail({ r, data, me, onClose, onAnswer, onUser }) {
  const [txt, setTxt] = useState("");
  return (
    <Sheet onClose={onClose} title="Ask the Community">
      <RequestCard r={r} users={data.users} me={me} dist={null} onOpen={() => {}} onReport={() => {}} onUser={onUser} />
      <SectionTitle>Answers</SectionTitle>
      {r.answers.length === 0 && <div style={{ color: T.inkFaint, fontSize: 13.5, padding: "4px 2px 12px" }}>Nobody Answer yet — help if you know!</div>}
      ...
      {r.status !== "expired" && <Composer value={txt} setValue={setTxt} placeholder="Write a helpful answer…" onSend={() => { if (txt.trim()) { onAnswer(r.id, txt.trim()); setTxt(""); } }} sendLabel="Answer" />}
    </Sheet>
  );
}
...
function CreateStatus({ me, myLoc, groups, presetGroup, onClose, onCreate, canPost, cooldown }) {
  ...
  const pickPhoto = (e) => {
    ...
    if (file.size > 12 * 1024 * 1024) { setMediaErr("Image too large (max 12MB)."); e.target.value = ""; return; }
  ...
  const pickVideo = (e) => {
    ...
    v.onloadedmetadata = () => {
      if (v.duration > MAX_VIDEO_SEC + 0.5) {
        setMediaErr(`Video too long — max ${MAX_VIDEO_SEC} seconds.`);
    ...
    v.onerror = () => { setMediaErr("Could not load video."); URL.revokeObjectURL(src); };
    v.src = src;
    e.target.value = "";
  };
  ...
  const submit = () => {
    setErr("");
    if (!canPost) return setErr(`To prevent spam, wait ${cooldown} seconds before posting again.`);
    if (text.trim().length < 3) return setErr("Write at least a few words about what's happening.");
    onCreate({
  ...
  return (
    <Sheet onClose={onClose} title="What's happening now?">
      <textarea style={{ ...inputStyle, minHeight: 90, resize: "none" }} placeholder="Share a real-time update — what, where and when…" value={text} onChange={(e) => setText(e.target.value)} maxLength={280} />
      ...
      <Field label="Location (optional)" hint="e.g. 5th Ave, New York">
        <input style={inputStyle} value={locName} onChange={(e) => setLocName(e.target.value)} placeholder="Where is this happening?" />
      </Field>

      <Field label="Media (optional)" hint="Attach a photo or short video up to 15 seconds.">
        {!media ? (
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={() => photoRef.current?.click()} style={uploadBtnStyle}><ImageIcon size={18} /> Upload photo</button>
            <button onClick={() => videoRef.current?.click()} style={uploadBtnStyle}><Video size={18} /> Video (15s)</button>
          </div>
        ) : (
      ...
      {err && <div style={{ background: "#FEECEC", color: T.live, borderRadius: 10, padding: "9px 12px", fontSize: 13, marginBottom: 12, fontWeight: 600 }}>{err}</div>}
      <PrimaryButton onClick={submit}><Radio size={17} /> Post Update</PrimaryButton>
    </Sheet>
  );
}
...
function CreateRequest({ me, myLoc, onClose, onCreate, canPost, cooldown }) {
  ...
  const submit = () => {
    setErr("");
    if (!canPost) return setErr(`To prevent spam, wait ${cooldown} seconds before posting again.`);
    if (text.trim().length < 5) return setErr("Please write a clearer question.");
  ...
  return (
    <Sheet onClose={onClose} title="Ask the Community">
      <div style={{ fontSize: 13.5, color: T.inkSoft, marginBottom: 12 }}>Ask the community what's happening at a specific place right now.</div>
      <textarea style={{ ...inputStyle, minHeight: 80, resize: "none" }} placeholder="e.g. How crowded is the beach? Any parking nearby?" value={text} onChange={(e) => setText(e.target.value)} maxLength={200} />
      <div style={{ height: 12 }} />
      <Field label="Category">
        <div className="sn-scroll-x" style={{ display: "flex", gap: 7, overflowX: "auto", paddingBottom: 4 }}>
          {CAT_KEYS.map((k) => <button key={k} onClick={() => setCat(k)} style={catBtn(cat === k, CATEGORIES[k].color)}>{React.createElement(CATEGORIES[k].Icon, { size: 13 })}{CATEGORIES[k].short}</button>)}
      ...
      <Field label="Which location?">
        <input style={inputStyle} value={locName} onChange={(e) => setLocName(e.target.value)} placeholder="e.g. Central Park, New York" />
      </Field>
      {err && <div style={{ background: "#FEECEC", color: T.live, borderRadius: 10, padding: "9px 12px", fontSize: 13, marginBottom: 12, fontWeight: 600 }}>{err}</div>}
      <PrimaryButton onClick={submit}><HelpCircle size={17} /> Post Question</PrimaryButton>
    </Sheet>
  );
}
...
function CreateGroup({ me, onClose, onCreate }) {
  ...
  const submit = () => {
    setErr("");
    if (name.trim().length < 2) return setErr("Please give the group a name.");
    onCreate({ name: name.trim(), description: desc.trim() || "New community", cover: COVERS[cover], privacy, memberIds: [me.id] });
  };
  return (
    <Sheet onClose={onClose} title="New community">
      <Field label="Group name"><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Brooklyn Heights Neighbors" /></Field>
      <Field label="Description"><textarea style={{ ...inputStyle, minHeight: 70, resize: "none" }} value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="What is this group about?" /></Field>
      ...
      <Field label="Privacy">
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={() => setPrivacy("public")} style={{ ...chipBtn(privacy === "public"), flex: 1, justifyContent: "center" }}><Globe size={15} /> Public</button>
          <button onClick={() => setPrivacy("private")} style={{ ...chipBtn(privacy === "private"), flex: 1, justifyContent: "center" }}><Lock size={15} /> Private</button>
        </div>
      </Field>
      {err && <div style={{ background: "#FEECEC", color: T.live, borderRadius: 10, padding: "9px 12px", fontSize: 13, marginBottom: 12, fontWeight: 600 }}>{err}</div>}
      <PrimaryButton onClick={submit}><Plus size={17} /> Create Group</PrimaryButton>
    </Sheet>
  );
}
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 40, background: "rgba(20,22,28,.35)", display: "flex", flexDirection: "column", justifyContent: "flex-end" }} onClick={onClose}>
      <div style={{ padding: "0 16px calc(96px + env(safe-area-inset-bottom))" }} onClick={(e) => e.stopPropagation()}>
        <button onClick={onStatus} style={menuItem(T.brand)}><Radio size={22} /><div style={{ textAlign: "start" }}><div style={{ fontWeight: 800, fontSize: 15 }}>Share a Status</div><div style={{ fontSize: 12.5, opacity: .9 }}>Share What's happening just now Nearby</div></div></button>
        <button onClick={onRequest} style={menuItem("#12A594")}><HelpCircle size={22} /><div style={{ textAlign: "start" }}><div style={{ fontWeight: 800, fontSize: 15 }}>Ask the Community</div><div style={{ fontSize: 12.5, opacity: .9 }}>Ask what's happening at a specific place</div></div></button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ position: "absolute", inset: 0, zIndex: 50, background: "rgba(20,22,28,.45)", display: "flex", alignItems: "center", justifyContent: "center", padding: 22 }}>
      <div style={{ background: T.surface, borderRadius: 22, padding: 24, maxWidth: 340, textAlign: "center" }}>
        ...
        <div style={{ fontWeight: 900, fontSize: 18 }}>Where are you?</div>
        <div style={{ fontSize: 13.5, color: T.inkSoft, marginTop: 8, lineHeight: 1.5 }}>We'll use location to show updates nearby. You can alsocation  or  Updates  Nearby.    Location .</div>
        <div style={{ marginTop: 18 }}>
          <PrimaryButton onClick={onGPS}><MapPin size={17} /> Share My Location</PrimaryButton>
          <button onClick={onManual} style={{ width: "100%", marginTop: 10, border: `1.5px solid ${T.line}`, background: T.surface, borderRadius: 13, padding: "12px", fontWeight: 800, fontSize: 14.5, cursor: "pointer", fontFamily: "inherit" }}>Choose location manually</button>
          <button onClick={onSkip} style={{ width: "100%", marginTop: 8, border: "none", background: "none", color: T.inkFaint, fontWeight: 700, fontSize: 13, cursor: "pointer" }}>Skip</button>
        </div>
      </div>
    </div>
  );
...
function ManualLocation({ onPick, onClose }) {
  return (
    <Sheet onClose={onClose} title="Choose Location">
      {CITIES.map((c) => (
...
function BottomNav({ tab, setTab, onCreate }) {
  const items = [
    { k: "feed", label: "Feed", Icon: Newspaper },
    { k: "map", label: "Map", Icon: MapIcon },
    { k: "requests", label: "Requests", Icon: HelpCircle },
...
/* ============================================================
   PERSISTENCE HELPERS
   ============================================================ */
const LS_SESSION = "sn_session_en_v1";
const LS_DATA    = "sn_data_en_v1";
...
export default function App() {
  ...
  const addAnswer = (id, text) => { setData((d) => ({ ...d, statuses: d.statuses, requests: d.requests.map((r) => r.id === id ? { ...r, status: "answered", answers: [...r.answers, { id: uid(), userId: meId, text, createdAt: now() }] } : r) })); showToast("Answer posted — thanks for helping! ⭐"); };
  ...
  const report = (id, kind) => showToast("Report received. Our team will review it. 🚩");

  const createStatus = (obj) => { setData((d) => ({ ...d, statuses: [{ id: uid(), ...obj }, ...d.statuses] })); lastPostRef.current = now(); setModal(null); showToast("Post published! 🚀"); };
  const createRequest = (obj) => { setData((d) => ({ ...d, requests: [{ id: uid(), ...obj }, ...d.requests] })); lastPostRef.current = now(); setModal(null); setTab("requests"); showToast("Question posted!"); };
  const createGroup = (obj) => { const id = uid(); setData((d) => ({ ...d, groups: [{ id, ...obj }, ...d.groups] })); setModal(null); setViewGroup(id); showToast("Group created! 🎉"); };
    ...
    const handleLogin = (userInfo) => {
      ...
      if (userInfo && userInfo.name) {
        ...
        const newUser = {
          ...
          bio: userInfo.provider === "anon" ? "Anonymous user 🎭" : userInfo.provider === "google" ? "Signed in with Google" : "New user",
          city: "",
  ...
  const titles = { feed: "Status Now", map: "Live Map", requests: "Ask Community", groups: viewGroup ? "Group" : "Groups", profile: viewUser && viewUser !== meId ? "Profile" : "My Profile" };
    ...
    <Shell>
      ...
      <header className="sn-topbar">
        ...
        <div className="sn-topbar-logo">
          ...
          <span style={{ fontSize:19, color:"#ffffff", fontWeight:900, letterSpacing:"-.3px" }}>Status<span style={{ color:"rgba(255,255,255,.7)" }}> Now</span></span>
        </div>
        {/* Center tabs — desktop */}
        ...
        <div className="sn-topbar-actions">
          <button onClick={()=>setModal({type:"createMenu"})} title="Post Update" style={{display:"flex",alignItems:"center",gap:7,border:"none",background:"rgba(255,255,255,.95)",color:"#0F766E",borderRadius:10,padding:"8px 16px",fontWeight:700,fontSize:14,cursor:"pointer",fontFamily:"inherit",border:"none",boxShadow:"0 1px 4px rgba(0,0,0,.12)",transition:"all .12s"}}><Plus size={17} strokeWidth={2.6}/>Post</button>
          <button className="sn-topbar-btn" onClick={()=>goTab("profile")} title="Profile"><Avatar user={me} size={36}/></button>
        </div>
      </header>
      ...
      <div className="sn-page">
        ...
        <aside className="sn-sidebar">
          ...
          <nav>
            {[
              { k:"feed",    label:"Feed",           Icon:Newspaper  },
              { k:"map",     label:"Live Map",   Icon:MapIcon    },
              { k:"requests",label:"Ask Community",    Icon:HelpCircle },
              { k:"groups",  label:"Groups",        Icon:Users      },
              { k:"profile", label:"My Profile",   Icon:UserIcon   },
            ].map(({k,label,Icon})=>(
              ...
              <div className="sn-compose">
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                  ...
                  <button className="sn-compose-btn" onClick={()=>setModal({type:"createMenu"})}>
                    What's happening near you, {me.name.split(" ")[0]}?
                  </button>
                </div>
                {/* action buttons row */}
              ...
              return (
                <div key={k} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:"1px solid #BFDBFE",cursor:"pointer"}} onClick={()=>setCatFilter(catFilter===k?null:k)}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    ...
                    <div style={{fontSize:11,color:"#94A3B8"}}>{count} active</div>
                  </div>
            ...
            {[...data.users].sort((a,b)=>b.reputation-a.reputation).slice(0,4).map(u=>(
              <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 0",borderBottom:"1px solid #BFDBFE",cursor:"pointer"}} onClick={()=>{openUser(u.id);}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  ...
                  <div style={{fontSize:11,color:"#94A3B8"}}>{u.city} · {u.reputation} pts</div>
                </div>
      ...
      {currentModal?.type === "manualLoc" && <ManualLocation onClose={() => setModal(null)} onPick={(c) => { setMyLoc({ ...c, name: c.name }); setAskedLoc(true); setModal(null); showToast(`Location updated: ${c.name} 📍`); }} />}
...
function Shell({ children }) {
  return (
    <div lang="en" style={{ minHeight: "100vh", background: "#EFF6FF", fontFamily: "system-ui,-apple-system,'Segoe UI','Helvetica Neue',sans-serif" }}>
      <style>{`
